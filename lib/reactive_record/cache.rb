  ...
  class Cache
    ...
    def fetch(klass, find_by, value, *associations)
      if RUBY_ENGINE != 'opal'
        ...
        React::RenderingContext.element_loading! # inform react that the current value is bogus
    ...
    def schedule_fetch
      @fetch_scheduled ||= after(0.0001) do
        #puts "starting fetch"
        ...
        HTTP.post("/reactive_record", payload: {pending_fetches: @pending_fetches.uniq}).then do |response| 
          ...
          React::State.set_state self, :last_fetch_at, last_fetch_at
