module ReactiveRecord
  class Base
    ...
    def self.data_interface
      @data_interface ||= PrerenderDataInterface.new
    end
    ...
    def self.load_from_json(json)
      @data_loading = true
      ServerDataCache.load_from_json(json)
      @data_loading = false
    end
    ...
    def self.find(model, attribute, value)  
      ...
      (records[model] = record = new(model)).vector = [model, ["find_by_#{attribute}", value]] unless record
      # and set the value
      ...
      record.ar_instance ||= model.new(record)
    ...
    def self.new_from_vector(model, aggregate_parent, *vector)
      ...
      (records[model] = record = new(model)).vector = vector unless record
      record.ar_instance ||= model.new(record)
    ...
    def id=(value)
      ...
      if existing_record
        ...
        existing_record.merge attributes
      else
    ...
    def save(&block) 
      if data_loading?
        ...
        while(record_index < records_to_process.count)
          ...
          output_attributes = {}
          models << {id: record.object_id, model: record.model.model_name, output_attributes}
    ...
    def self.infer_type_from_hash(klass, hash)
      ...
      # this is just a helper used by other classes.  It lives here to keep the AR method name space clean.
