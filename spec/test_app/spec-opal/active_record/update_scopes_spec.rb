...
describe "updating scopes" do
  ...
  # the tests depend on each other.  Opal-rspec < 0.5 runs specs backwards, so we have to account for that.
  ...
  tests = tests.reverse unless sequenced_asyncs?

  tests.each { |test| test.call }
